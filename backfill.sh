...
if [[ -f /tmp/backfill ]]; then
    ...
    chainweb-data fill --service-host=104.248.86.98 --p2p-host=104.248.86.98 --service-port=8080 --p2p-port=1917 --dbuser=postgres --dbpass=postgres --dbname=postgres
...
until [[ "$x" == 1 ]] ; do
  ...
  if [[ "$server_check" == 2 ]]; then
    ...
    chainweb-data fill --service-host=104.248.86.98 --p2p-host=104.248.86.98 --service-port=8080 --p2p-port=1917 --dbuser=postgres --dbpass=postgres --dbname=postgres +RTS -N
    sleep 10
