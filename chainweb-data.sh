...
echo -e "Waiting for chainwebdata_build marker..."
 ...
 if [[ -f /usr/local/bin/chainweb-data ]]; then
   node_await
   chainweb-data server --port 8888 --service-host=104.248.86.98 --p2p-host=104.248.86.98 --service-port=8080 --p2p-port=1917 --dbuser=postgres --dbpass=postgres --dbname=postgres -m
   exit
 fi
